...
import cl.mybatis.pojos.ModeloExe;
 ...
 * @author user1
 */
@ManagedBean(name="ModeloExeBean")
@ViewScoped
// @RequestScoped

public class ModeloExeBean implements Serializable  {

    private ModeloExe formato;
    private List<ModeloExe> listFormatos; 
    private ModeloExe formatoSelected; 
    ...
    @PostConstruct
    public void init() {
        System.out.println(" INIT ModeloExe----------------------------------------------------------------------");
         formato = new ModeloExe();
         formatoSelected = new ModeloExe();
  ...
  // formatoModeloExe.desplegar();
    ...
    public ModeloExeBean() {
    }

    public ModeloExeBean(ModeloExe formato, List<ModeloExe> listFormatos, ModeloExe formatoSelected) {
        this.formato = formato;
        this.listFormatos = listFormatos;
        this.formatoSelected = formatoSelected;
    }

    public ModeloExe getFormato() {
        return formato;
    }

    public void setFormato(ModeloExe formato) {
        this.formato = formato;
    }

    public List<ModeloExe> getListFormatos() {
        return listFormatos;
    }

    public void setListFormatos(List<ModeloExe> listFormatos) {
        this.listFormatos = listFormatos;
    }

    public ModeloExe getFormatoSelected() {
        return formatoSelected;
    }

    public void setFormatoSelected(ModeloExe formatoSelected) {
        this.formatoSelected = formatoSelected;
    }
    ...
    public void selectFilter(ModeloExe pModeloExe) throws Exception {
        SqlSession session = new myBatisUtil().getSession();
        System.out.println("----------------------------------------------------------------------");
        System.out.println("Ingreso Select Nombre ModeloExe  "+ pModeloExe.getMexe_secue() );
        
        if ( session != null ){
            try {
                
/*                if ( pModeloExe.getAgendaCode() == null || pFormatoIcbf.getAgendaCode().length() < 1 ){
                    pFormatoIcbf.setAgendaCode("4");
                }
*/                    
                
                this.listFormatos = session.selectList("ModeloExe.selectFilter", pModeloExe);
                if ( this.listFormatos.size() > 0 ) {
                  System.out.println(" ------ > selectFilter - Tamaño de la lista "+this.listFormatos.size()+ " Codigo "+ this.listFormatos.get(0).getMexe_secue()+" Nombre " );                    
      ...
      public void selectAll(){
          ...
          try {
            ...
            ModeloExe formatoParametro = new ModeloExe();
            
            formatoParametro.setMod_secue(formatoSelected.getMod_secue());
            selectFilter(formatoParametro);
            
        } catch (Exception ex) {
            Logger.getLogger(ModeloExeBean.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    public String update()  {
        ...
        // this.setMensaje("");
        SqlSession session = new myBatisUtil().getSession();
        System.out.println("Paso 1 - ModeloExe  Ingreso opcion de actualizacion  ");
        System.out.println("Paso 2 - ModeloExe  Ingreso opcion de actualizacion "+this.getEstadoCRUD());
        
        System.out.println("ModeloExe  "+this.formatoSelected.getMexe_secue());
        if ( session != null ){
            try {
              ...
              if ( this.getEstadoCRUD().equals("Nuevo")  )
                {
                   ...
                   this.setMensaje("ModeloExe Adicionado");
                ...
                else
                {
                   ...
                   session.update("ModeloExe.update", this.formatoSelected);     
                   System.out.println("Funcion Update - Actualizar .");
                   this.setMensaje("ModeloExe Actualizado");
...
// 1: Si encontro items con el mismo codigo    
    ...
    public int existeCodigo()  {
        ...
        ModeloExe buscarItem = new ModeloExe();
        List<ModeloExe> listItems; 

        String redireccion = null;
        this.setMensaje("");
        SqlSession session = new myBatisUtil().getSession();
        System.out.println("Verificar si codigo ModeloExe existe.");
        ...
        if ( session != null ){
            try {
                ...
                listItems = session.selectList("ModeloExe.selectFilter", buscarItem);
                   ...
                   if ( listItems.size() >= 1 ) {
                      ...
                      for ( int i = 0 ; i < listItems.size() ; i++  ) 
                      {
                           ...
                           if ( ! this.formatoSelected.getMod_secue().equals(listItems.get(i).getMod_secue()) )  {  // Si el ID es diferente entonces el codigo es igual genera error
                              this.setMensaje("Modificacion - Codigo ModeloExe ya existe. Cambiar el codigo.");
                              return 1; 
                           }
                ...
                if ( this.getEstadoCRUD().equals("Nuevo") ) {
                   if ( listItems.size() > 0 ) {
                      this.setMensaje("Nuevo - Codigo ModeloExe ya existe. Cambiar el codigo.");
                      return 1;      
                   }
                } // fin del nuevo     
...
// Buscar con filtros ----------
    public String selectFilterSelected()  {
        ...
        this.setMensaje("");
        SqlSession session = new myBatisUtil().getSession();
        System.out.println("Consulta ModeloExe");
        ...
        if ( session != null ){
            try {
                ...
                this.listFormatos = session.selectList("ModeloExe.selectFilter", this.formatoSelected);
                if ( this.listFormatos.size() > 0 ) {
    ...
    // -- Dispone de las variables para realizar la adicion
        ...
        System.out.println("Adicionar Registro ModeloExe ");
