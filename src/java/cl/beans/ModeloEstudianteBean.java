 ...
 * @author user1
 */
@ManagedBean(name="ModeloEstudianteBean")
@ViewScoped
// @RequestScoped

public class ModeloEstudianteBean implements Serializable  {

    private ModeloEstudiante formato;
    private List<ModeloEstudiante> listFormatos; 
    private ModeloEstudiante formatoSelected; 
    ...
    @PostConstruct
    public void init() {
        System.out.println(" INIT ModeloEstudiante----------------------------------------------------------------------");
         formato = new ModeloEstudiante();
         formatoSelected = new ModeloEstudiante();
  ...
  // formatoModeloEstudiante.desplegar();
    ...
    public ModeloEstudianteBean() {
    }

    public ModeloEstudianteBean(ModeloEstudiante formato, List<ModeloEstudiante> listFormatos, ModeloEstudiante formatoSelected) {
        this.formato = formato;
        this.listFormatos = listFormatos;
        this.formatoSelected = formatoSelected;
    }

    public ModeloEstudiante getFormato() {
        return formato;
    }

    public void setFormato(ModeloEstudiante formato) {
        this.formato = formato;
    }

    public List<ModeloEstudiante> getListFormatos() {
        return listFormatos;
    }

    public void setListFormatos(List<ModeloEstudiante> listFormatos) {
        this.listFormatos = listFormatos;
    }

    public ModeloEstudiante getFormatoSelected() {
        return formatoSelected;
    }

    public void setFormatoSelected(ModeloEstudiante formatoSelected) {
        this.formatoSelected = formatoSelected;
    }
    ...
    public void selectFilter(ModeloEstudiante pModeloEstudiante) throws Exception {
        SqlSession session = new myBatisUtil().getSession();
        System.out.println("----------------------------------------------------------------------");
        System.out.println("Ingreso Select Nombre ModeloEstudiante  "+ pModeloEstudiante.getMexe_secue() );
        
        if ( session != null ){
            try {
                
/*                if ( pModeloEstudiante.getAgendaCode() == null || pFormatoIcbf.getAgendaCode().length() < 1 ){
                    pFormatoIcbf.setAgendaCode("4");
                }
*/                    
                
                this.listFormatos = session.selectList("ModeloEstudiante.selectFilter", pModeloEstudiante);
                if ( this.listFormatos.size() > 0 ) {
                  System.out.println(" ------ > selectFilter - Tamaño de la lista "+this.listFormatos.size()+ " Codigo "+ this.listFormatos.get(0).getEst_id_secue()+" Nombre " );                    
      ...
      public void selectAll(){
          ...
          try {
            ...
            ModeloEstudiante formatoParametro = new ModeloEstudiante();
            
            formatoParametro.setMod_secue(formatoSelected.getMod_secue());
            selectFilter(formatoParametro);
            
        } catch (Exception ex) {
            Logger.getLogger(ModeloEstudianteBean.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    public String update()  {
        ...
        // this.setMensaje("");
        SqlSession session = new myBatisUtil().getSession();
        System.out.println("Paso 1 - ModeloEstudiante  Ingreso opcion de actualizacion  ");
        System.out.println("Paso 2 - ModeloEstudiante  Ingreso opcion de actualizacion "+this.getEstadoCRUD());
        
        System.out.println("ModeloEstudiante  "+this.formatoSelected.getMexe_secue());
        if ( session != null ){
            try {
              ...
              if ( this.getEstadoCRUD().equals("Nuevo")  )
                {
                   ...
                   session.insert("ModeloEstudiante.insert", this.formatoSelected);
                   System.out.println("Funcion Update - Insert .");
                   this.setMensaje("ModeloEstudiante Adicionado");
                ...
                else
                {
                   ...
                   session.update("ModeloEstudiante.update", this.formatoSelected);     
                   System.out.println("Funcion Update - Actualizar .");
                   this.setMensaje("ModeloEstudiante Actualizado");
...
// 1: Si encontro items con el mismo codigo    
    ...
    public int existeCodigo()  {
        ...
        ModeloEstudiante buscarItem = new ModeloEstudiante();
        List<ModeloEstudiante> listItems; 

        String redireccion = null;
        this.setMensaje("");
        SqlSession session = new myBatisUtil().getSession();
        System.out.println("Verificar si codigo ModeloEstudiante existe.");
        System.out.println("Buscar Codigo "+this.formatoSelected.getMexe_secue()+" Id Secue  "+this.formatoSelected.getMod_secue());
        ...
        if ( session != null ){
            try {
                ...
                listItems = session.selectList("ModeloEstudiante.selectFilter", buscarItem);
                   ...
                   if ( listItems.size() >= 1 ) {
                      ...
                      for ( int i = 0 ; i < listItems.size() ; i++  ) 
                      {
                           ...
                           if ( ! this.formatoSelected.getMod_secue().equals(listItems.get(i).getMod_secue()) )  {  // Si el ID es diferente entonces el codigo es igual genera error
                              this.setMensaje("Modificacion - Codigo ModeloEstudiante ya existe. Cambiar el codigo.");
                              return 1; 
                           }
                ...
                if ( this.getEstadoCRUD().equals("Nuevo") ) {
                   if ( listItems.size() > 0 ) {
                      this.setMensaje("Nuevo - Codigo ModeloEstudiante ya existe. Cambiar el codigo.");
                      return 1;      
                   }
                } // fin del nuevo     
...
// Buscar con filtros ----------
    public String selectFilterSelected()  {
        ...
        this.setMensaje("");
        SqlSession session = new myBatisUtil().getSession();
        System.out.println("Consulta ModeloEstudiante");
        System.out.println("Buscar Codigo "+this.formatoSelected.getMod_secue()+" Nombre  "+this.formatoSelected.getMexe_secue());
        ...
        if ( session != null ){
            try {
                ...
                this.listFormatos = session.selectList("ModeloEstudiante.selectFilter", this.formatoSelected);
                if ( this.listFormatos.size() > 0 ) {
                  System.out.println(" ------ > selectFilter - Tamaño de la lista "+this.listFormatos.size()+ " Nombre Usuario "+ this.listFormatos.get(0).getMexe_secue()+" Codigo "+ this.listFormatos.get(0).getMod_secue() );                    
                  this.formato = this.listFormatos.get(0);                  
                  this.formatoSelected = this.listFormatos.get(0);                  
                  redireccion = "";
    ...
    // -- Dispone de las variables para realizar la adicion
        ...
        System.out.println("Adicionar Registro ModeloEstudiante ");
...
public static void main(String arg[]) throws Exception {
  ...
  System.out.println("ModeloEstudiante Perfil - TEST ");
  
  ModeloEstudiante formato = new ModeloEstudiante();
  
  
  

  ModeloEstudianteBean formatoBean = new ModeloEstudianteBean();
